...
const ExportType = props => (
  <div>
    ...
    ? (
      <Button
        className="ExportBack"
        label="Go back"
        onClick={props.removeChoice}
      />
    ) : false}
    <div className={`ExportType ${props.expanded ? 'expanded' : ''}`} role="button" tabIndex="0" onClick={props.expanded ? () => {} : props.selectChoice}>
      <div className="card">
        <div className="image" />
      ...
      <div className="description">
        ...
        >
          <Button
            className="select"
            label="Go for it"
            onClick={() => { props.storeExportType(props.choice.type); }}
          />
        </Checkout>
      </div>
    </div>
