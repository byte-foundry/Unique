...
export const loadFont = (f, step, redirectTo = '/app/specimen') => (dispatch, getState) => {
	...
	Promise.all(promiseArray).then(() => {
		...
		request(GRAPHQL_API, getSelectedCount('Preset', font.id)).then((data) =>
			request(
				GRAPHQL_API,
...
const getCalculatedValues = (choice, choicesMade, currentPreset) => {
	...
	[...choiceWithoutCurrentStep, stepChoices].forEach((choiceMade) => {
		choiceSpecialPropsKeys.push(
			...Object.keys(choiceMade.glyphSpecialProps || {}),
		);
	});
	const baseSpecialPropsGlyphs = uniq(
		...[Object.keys(baseSpecialProps), ...choiceSpecialPropsKeys],
	);
		...
		case currentPreset.steps.length + 1:
			...
			try {
				...
				ga(
					...
					'Font',
					'Finished',
					currentPreset.steps[step - 1].name,
				);
			} catch (e) {}
...
export const selectChoice = (choice, isSpecimen = false) => (
	...
	if (
		...
		request(GRAPHQL_API, getSpecialChoiceSelectedCount(choice.name)).then(
			(data) =>
				request(
					GRAPHQL_API,
					updateSelectedCount(
		...
		request(GRAPHQL_API, getSelectedCount('Choice', choice.id)).then((data) =>
			request(
				GRAPHQL_API,
				updateSelectedCount(
					'Choice',
					choice.id,
					data.Choice ? data.Choice.selected + 1 : 1,
				),
			),
		);
	}
...
export const createFontVariants = () => (dispatch, getState) => {
	...
	const possibleWidth = ['CONDENSED', 'NORMAL', 'EXTENDED'];
	const possibleSlant = ['NORMAL', 'SLANTED'];
	//  --  Create thickness variant
	const thicknessStep = currentPreset.steps.find(
		(e) => e.name.toUpperCase() === 'THICKNESS',
	);
	const widthStep = currentPreset.steps.find(
		(e) => e.name.toUpperCase() === 'WIDTH',
	);
	const slantStep = currentPreset.steps.find(
		(e) => e.name.toUpperCase() === 'SLANT',
	);
	...
	if (thicknessStep) {
		...
		const thicknessChoiceIndex = currentPreset.steps.findIndex(
			(e) => e.name.toUpperCase() === 'THICKNESS',
		);
		...
		const filteredThicknessVariantPossibilities = thicknessVariantPossibilities.filter(
			(e) => e.name !== choicesMade[thicknessChoiceIndex].name,
		);
		const thicknessChoicesToKeep = choicesMade.filter(
			(e, index) => e && index !== thicknessChoiceIndex,
		);
	...
	if (widthStep) {
		...
		const widthChoiceIndex = currentPreset.steps.findIndex(
			(e) => e.name.toUpperCase() === 'WIDTH',
		);
		...
		const filteredWidthVariantPossibilities = widthVariantPossibilities.filter(
			(e) => e.name !== choicesMade[widthChoiceIndex].name,
		);
		const widthChoicesToKeep = choicesMade.filter(
			(e, index) => e && index !== widthChoiceIndex,
		);
	...
	if (slantStep) {
		...
		const slantChoiceIndex = currentPreset.steps.findIndex(
			(e) => e.name.toUpperCase() === 'SLANT',
		);
		// Check which possibleSlant are in the presets
		...
		// Remove selected slant
		const filteredSlantVariantPossibilities = slantVariantPossibilities.filter(
			(e) => e.name !== choicesMade[slantChoiceIndex].name,
		);
		const slantChoicesToKeep = choicesMade.filter(
			(e, index) => e && index !== slantChoiceIndex,
		);
		// Create fonts - Apply currentParams then slant choice values
