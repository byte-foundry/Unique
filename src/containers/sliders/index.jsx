  ...
  choices.forEach(choice => {
    Object.keys(choice.values).forEach(key => {
      if (
        ...
        key !== "manualChanges" &&
        key !== "name" &&
        key !== "glyphComponentChoice" &&
        key !== "glyphSpecialProps" &&
        key !== "indiv_group_param" &&
        key !== "indiv_glyphs" &&
        key !== "altList"
      ) {
        customParams.push({
          name: params[key].title,
      ...
      <div className="Sliders">
        <div className="row">
          {this.state.params.map(param => (
            <div className="slider col-lg-6" key={param.name}>
              ...
              <Slider
                ...
                tooltip={false}
                onChange={value => {
                  this.onSliderChange(value, param.key);
                }}
              />
            </div>
          ))}
...
const mapStateToProps = state => ({
  currentFontValues: {
    ...state.font.stepBaseValues,
    ...state.font.currentParams
  },
  choices: state.font.currentPreset.steps[state.font.step - 1].choices,
  controls: state.createdFonts.fonts[state.font.fontName]
    ? state.createdFonts.fonts[state.font.fontName].json.controls
    : []
});

const mapDispatchToProps = dispatch =>
  bindActionCreators({ resetSliderFont }, dispatch);
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Sliders)
);
